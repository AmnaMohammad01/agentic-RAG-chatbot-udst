...
index, all_chunks, valid_policies = None, None, fetch_policies()
...
policy_list = "\n".join([f"- [{title}]({url})" for title, url in valid_policies.items()])
st.markdown(f"<div style='height: 300px; overflow-y: auto; border: 1px solid #ddd; padding: 10px;'>{policy_list}</div>", unsafe_allow_html=True)
